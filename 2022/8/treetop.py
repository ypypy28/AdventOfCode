...
def check_top(
    ...
    for x in range(1, len(buffer[-1])-1):
        size = buffer[-1][x]
        if size > top[x].size:
            upper = [size > buffer[j][x] for j in range(len(buffer)-2, -1, -1)]
            visible = not upper or all(upper)
            if visible:
                visible_inside.add(Tree(x, line_i, size))
            ...
            if bottom[x] is not None:
                ...
                for j in range(len(buffer)-1, -1, -1):
                    ...
                    if buffer[j][x] > max_height:
                        visible_inside.add(Tree(x,
                                                line_i-len(buffer)+1+j,
                                                buffer[j][x]))
...
def main():
    ...
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            right.append(Tree(len_prev-1, y, buf[-1][-1]))
    ...
    bottom[:len(buf[-1])] = [Tree(x, y, size) for x, size in enumerate(buf[-1])]
    all_edges.update(bottom)
    buf.pop()
